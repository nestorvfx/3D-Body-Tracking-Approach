"""Render character WITHOUT clothes to inspect raw mesh deformation."""
import sys, math
...
bvh_path = args[0]
hdri_path = args[1]
out_dir = Path(args[2]) if len(args) > 2 else HERE.parent / "output" / "naked"
...
for f in [100, 142, 180, 220, 260]:
    ...
    cam_obj.location = (root_w.x, root_w.y - 3.0, root_w.z + 0.3)
    fwd = mathutils.Vector((0, 1, -0.05)).normalized()
    cam_obj.rotation_euler = fwd.to_track_quat("-Z", "Y").to_euler()
    bpy.context.scene.render.filepath = str(out_dir / f"FRONT_{f:04d}.png")
    bpy.ops.render.render(write_still=True)

    cam_obj.location = (root_w.x, root_w.y + 3.0, root_w.z + 0.3)
    fwd = mathutils.Vector((0, -1, -0.05)).normalized()
    cam_obj.rotation_euler = fwd.to_track_quat("-Z", "Y").to_euler()
    bpy.context.scene.render.filepath = str(out_dir / f"REAR_{f:04d}.png")
    bpy.ops.render.render(write_still=True)

    cam_obj.location = (root_w.x + 3.0, root_w.y, root_w.z + 0.3)
    fwd = mathutils.Vector((-1, 0, -0.05)).normalized()
    cam_obj.rotation_euler = fwd.to_track_quat("-Z", "Y").to_euler()
    bpy.context.scene.render.filepath = str(out_dir / f"SIDE_{f:04d}.png")
    bpy.ops.render.render(write_still=True)
    print(f"Saved frame {f}")
