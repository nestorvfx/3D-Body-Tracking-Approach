...
def sample_focal_mm(rng) -> float:
    """
    BEDLAM2.0 spans 14-400mm log-frequency. Most of the mass sits in the
    phone/DSLR range (~24-85mm). We approximate with a lognormal centred at
    ln(35) with sigma ~0.55, then clamp to [14, 400].
    """
    mu, sigma = math.log(35.0), 0.55
...
def frame_armature_strict(camera: bpy.types.Object,
    ...
    intr_shift_x = rng.uniform(-0.03, 0.03)
    intr_shift_y = rng.uniform(-0.03, 0.03)
