...
def sample_phenotype(rng: random.Random) -> Phenotype:
    """Sample an athletic/average phenotype.  MPFB macros at defaults
    produce pregnant-belly + oversized-glute shapes that fail a judge's
    visual plausibility test.  Narrower weight/proportions ranges and
    elevated muscle push the silhouette toward athletic.
    """
    r = [rng.random() for _ in range(3)]
    s = sum(r) or 1.0
    return Phenotype(
        gender=rng.uniform(0.05, 0.95),      # FULL range: feminine to masculine
        age=rng.uniform(0.25, 0.80),         # young adults to seniors
        muscle=rng.uniform(0.25, 0.85),
        weight=rng.uniform(0.15, 0.60),      # lean to average; avoid extreme-BMI artifacts
        height=rng.uniform(0.20, 0.90),      # short to tall
        proportions=rng.uniform(0.30, 0.70),
        cupsize=rng.uniform(0.20, 0.70),
        firmness=rng.uniform(0.40, 0.85),
...
def build_character(seed: int, with_assets: bool = True) -> tuple[object, object]:
    ...
    # subdiv_levels 2 = 1 level on base (smooths face polygons so it doesn't
    # read as blocky CG).  Costs a little render time but is essential for
    # BEDLAM-level realism.
